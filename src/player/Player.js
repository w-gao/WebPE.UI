...
export default class Player {
    ...
    constructor(opts) {
        ...
        this.player_color = opts.player_color;
        this.player_height = opts.player_height;

        this.update_player_skin = true;
        this.walk_speed = 0.0;
        this.max_walk_speed = 1.0;
        this.started_walking = 0.0;
        this.stopped_walking = 0.0;
        this.walking = false;
        this.acceleration = 1.0;

        let BABYLON = opts.BABYLON;
        ...
        let player = new BABYLON.SolidParticleSystem("Player", scene);
        ...
        this.player_mesh = player.buildMesh();
        ...
        let started_walking = this.started_walking;
        let stopped_walking = this.stopped_walking;
        let max_walk_speed = this.max_walk_speed;
        let walk_speed = this.walk_speed;
        let acceleration = this.acceleration;
        let player_color = this.player_color;

        // Player animation
        player.updateParticle = (particle) => {
            let time = Date.now() / 1000
            if (walking && time < started_walking + acceleration) {
                this.walk_speed = (time - started_walking) / acceleration;
            }
            if (!walking && time < stopped_walking + acceleration) {
                this.walk_speed = -1 / acceleration * (time - stopped_walking) + 1;
            }

            // Range-check
            this.walk_speed = Math.min(Math.abs(walk_speed), max_walk_speed);
    ...
    start_walking() {
        ...
        if (stopped_walking + acceleration > now) {
            ...
            this.started_walking = now - (stopped_walking + acceleration - now);
        } else {
            this.started_walking = Date.now() / 1000;
        }
    }
    ...
    stop_walking() {
        ...
        if (started_walking + acceleration > now) {
            this.stopped_walking = now - (started_walking + acceleration - now);
        } else {
            this.stopped_walking = Date.now() / 1000;
        }
    }
